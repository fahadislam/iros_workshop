...
However, this may at times incur large online planning times that may be unacceptable in many settings.
%
...
We demonstrate a speedup of over tenfold in query time when compared to the \textsf{PRM} algorithm with a memory footprint o less than 8 Mb while guaranteeing a maximal query time of less than 3 milliseconds.
...
However, as we will see in our evaluation (Sec.~\ref{sec:eval}), our algorithm typically covers most of the goal region $G_\calS$ using a few very large regions (namely, with a large radii~$r_i$) and the rest of $G_\calS$ is covered by a number of very small regions.
...
Unfortunately, this may require the algorithm to store many states not in $G_\calS$  which may be impractical (recall that Assumption~\ref{assum:1} only states that we can exhaustively cover the states in the goal region).
