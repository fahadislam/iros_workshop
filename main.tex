...
\begin{abstract} 
In many manipulation tasks, robots often have to perform highly-repetitive manipulation tasks in structured environments e.g. sorting mail in a mailroom or pick and place objects off a conveyor belt.
...
If we precompute each path $\pi_i \in \calL$ with the optimal cost $c^*(\pi_i)$, from triangular inequality it can be trivially shown that the quality of complete path $\pi$ computed by our method has an additive suboptimality bound,
 \begin{center}
 $c(\pi) - c^*(\pi) < 2 * c(\pi_g)$,
 \end{center}
