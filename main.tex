...
While the environment does not change, the start and goal of each repetitive task is similar, yet not identical, to previous tasks.
Difference in the exact start and goal position may be due to uncertainty in the environment as objects may be placed on different parts of the conveyor belt or in different orientation.
...
This insight allows us to generate offline a small set of so-called ``attractor vertices'' together with a path between each attractor state and the goal.
...
% combine a precomputed library~$\calL$ of paths between \emph{several} start and goal configurations together with a method to connect \emph{any} start and goal configuration to a path in~$\calL$ \emph{without} having to perform collision detection.
...
Finally, our notion of attractor states bares resemblance to control-based methods that  ensure safe operation over local regions of the free configuration space~\cite{CRC03,CCR06}.
...
\section {Analysis}
{\color{blue} This is wrong unless we talk about tie breaking. I don't think that tie breaking is an implementation detail and we should mention it in the prev. section}
...
{\color{blue} Add branching factor.}
