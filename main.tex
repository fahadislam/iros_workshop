  ...
  % \begin{center}
    \resizebox{\textwidth}{!}{%
      \begin{tabular}{ l | c c c  c c c  c c c  c  c}
       ...
       \hline
       Planning time [ms]& 15.2 (25.5) & - (-) & - (-) & 15.6 (24.9) & 16.8 (28.0) & 16.9 (30.1) & - (-) & 459.1 (3063.65) & 0.42 (0.7)\\
       Success rate [$\%$]& 98 & - & - & 90.5 & 88 & 89.5 & - & 98 & 100\\
       Memory usage [Mb] & 724 & - & - & 118 & 183 & 284 & - & - & 7.8\\
      \end{tabular}
  }
...
For our method, preprocessing (Alg.~\ref{alg:1}) took 1,569 seconds and returned 504 subregions. For precomputing paths (Alg.~\ref{alg:1}, line~\ref{alg:1:pp}), we use \textsf{RRT}-Connect. The preprocessing time for our methods is predominantly consumed in precomputing the library of paths $\calL$. The planning times of \textsf{RRT}-Connect show a very high variance ((see Table~\ref{tab:stats})). To optimise for the preprocessing time, we do the following trick. We initially set a small timeout (10 seconds) for \textsf{RRT}-Connect. The attractor states for which \textsf{RRT}-Connect fails to find a path to $\sStart$, we mark them as $\textit{bad}$ attractors and do not grow the subregions from them. We procedurally remove these bad attractors from the list as the other subregions cover them. When Alg.~\ref{alg:1} terminations we reload the valid list $V$ with all the remaining bad attractors and rerun (Alg.~\ref{alg:1}) but this time with a large timeout (60 seconds) for \textsf{RRT}-Connect. By doing so the algorithm finishes successfully without having any remaining bad attractor.
