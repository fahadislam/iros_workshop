...
Alternatively, this may be bacuse we prun away states that are in $G_\calS$. 
However, using the fact that our goal region is conves with respect to $h$ (Assumption~~\ref{assum:2}), this cannot hold.
\end{proof}
...
Unfortunately, this may require the algorithm to store many states not in $G_\calS$  which may be impractical (recall that Assumption~\ref{assump:1} only states that we can exhaustively cover the states in the goal region).
  ...
  % }
    ...
    % \resizebox{1.4\textwidth}{!}{\begin{minipage}{\textwidth}
        \begin{tabular}{ l | c c c c c}
         ...
         Memory usage [Mb] & 1,828 & 225.75 & 2.0 & - & \textbf{7.8}
        \end{tabular}
    }
    \caption{Experimental results comparing our method with other single- and multi-query planners. The table shows the mean/worst-case planning times, success rates and memory usage for our method and for other multi-query planners preprocessed with quadruple the time that our method takes in precomputation (T = 1,569 seconds). Note that the worst-case time for our method shown in these results ($\sim$1.6 millisecond) is the empirical one and not the computed provable time bound which is 3 milliseconds for this environment.
...
\begin{figure*}
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        ...
        \label{fig:memory}
    \end{subfigure}
    \caption{The plots shows the preprocessing time versus (\subref{fig:success}) the success rates and (\subref{fig:memory}) for the 200 queries averaged over 4 trials with different random number generation seeds for the PRM and the MQ-RRT algorithms. The results were computed at the intervals which are multiple of the time T = 1,455 seconds, that our method takes for precomputation. The red cross shows our method for reference.}
    \label{fig:plots}
\end{figure*}
