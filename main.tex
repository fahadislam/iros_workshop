...
Assumption~\ref{assum:2} may be seen as restrictive, imposing that the goal region can not be of arbitrary structure. However, after we detail our algorithm, we sketch how we can slightly relax this assumption to be more realistic.
...
\textbf{Remark}: 
\update{We can relax Assumption~\ref{assum:2} such that the heuristic function is not  weakly monotone with respect to $G_calS$ but with respect to all states.
