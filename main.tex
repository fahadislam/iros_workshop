...
\While{OPEN$\neq \emptyset$}
    ...
    \State $s'_g \leftarrow \arg \min_{s' \in \text{Preds}(s)} h(s', \sAttract)$ 
...
\Procedure{FindGreedyPath}{$s_1, s_2$}
	...
	\While{$s_{\text{curr}} \neq s_2$}
    	...
    	\State $s_{\text{curr}} \leftarrow \arg \min_{s \in \text{Preds}(s_{\text{curr}})} h(s, s_2)$ 	\Comment{greedy succesor}
    \EndWhile
    \State \Return $\pi$
\EndProcedure
