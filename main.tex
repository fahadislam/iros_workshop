...
However, storing a path from $\sStart$ to each state in $G_\calS$ is infeasible.
 ...
 \begin{itemize}
  ...
  \begin{center}
     $\argmin\limits_{s_1' \in \text{Succs}(s_1)} h(s_1', s_2) \in G_\calS$.
  \end{center}

 \end{itemize}
 Namely, for any distinct pair of states ($s_1, s_2$) in $G_\calS$, at least one of $s_1$'s successors has a heuristic value less than or equal to its heuristic value.
...
(Assumption~\ref{assum:3}), then every state has a greedy successor in the $G_\calS$ and it is unique.
...
\begin{definition}
	...
	an algorithm is said to be a \emph{greedy search}	 with respect to $h$ if for every state it returns its greedy successor according to $h$.
\end{definition}
