...
However, storing a path from $\sStart$ to each state in $G_\calS$ is infeasible.
  ...
  \item \label{assum:2} The planner has access to a heuristic function $h: \calS \times \calS \rightarrow \mathbb{R}$ which can estimate the distance between any two states in $G_\calS$. Moreover the heuristic function should be $\textit{weakly-monotone}$ with respect to $G_\calS$, meaning that $\forall s_1, s_2  \in G_\calS$ where $s_1 \neq s_2 $, it holds that,
%  \begin{center}
...
%  \end{center}
  ...
  \end{center}
% Namely, for any distinct pair of states ($s_1, s_2$) in $G_\calS$, at least one of $s_1$'s successors  that also belonging to $G_\calS$ must have a heuristic value less than or equal to its heuristic value.
 Namely, for any distinct pair of states ($s_1, s_2$) in $G_\calS$, at least one of $s_1$'s successors  that also belonging to $G_\calS$ must have a heuristic value less than or equal to its heuristic value.
 Moreover, the successor with minimal heuristic valies lies in the goal region.
...
%a
%\newpage
%\update{The way Alg.~\ref{alg:2} is written (this is where we use the following assumption), we expand a subregion out side of the goal region. This can be good or bad, it can waist a lot of time as we are ``learning'' parts of the C-space that we don't care about but could allow us to have larger subregions.}
%%
%
%Assumption \textbf{A2}  The planner has access to a heuristic function $h: \calS \times \calS \rightarrow \mathbb{R}$ which can estimate the distance between any two states in $G_\calS$.
%
%\textbf{Option 1:}
%Moreover the heuristic function should be $\textit{weakly-monotonic}$, meaning that $\forall s_1, s_2  \in G_\calS$ where $s_1 \neq s_2 $, it holds that,
%  \begin{center}
%    $h(s_1, s_2) \geq \min\limits_{s_1' \in \text{Succs}(s_1) \cap G_\calS} h(s_1', s_2)$.
%  \end{center}
%\update{As Max pointed out, this means that we have some requirement over the goal region which is not ideal.
%However, this simplifies Alg.~\ref{alg:2} and is actually what we implemented.}
%
%\textbf{Option 2:}
%Moreover the heuristic function should be $\textit{weakly-monotonic}$, meaning that $\forall s_1, s_2  \in G_\calS$ where $s_1 \neq s_2 $, it holds that,
%  \begin{center}
%    $h(s_1, s_2) \geq \min\limits_{s_1' \in \text{Succs}(s_1)} h(s_1', s_2)$.
%  \end{center}
%\update{Here, the requirements on the heuristic function is less strict. We have two options for this case:
%(1) change the notion of a greedy successor (def~\ref{def:greedy-suc} to ensure that the greedy successor is within)}
...
or 
(ii)~the condition when the reachability search re-enters the goal  region (see Fig.~\ref{fig:alg2b}). 
...
\textbf{Remark}: Throughout this section we assumed that the underlying graph is symmetric (namely if there is an edge between $s$ and $s'$, then there is an edge between $s'$ and $s$). This was done for ease of exposition. In the general case, we would have to alter our reachability search (Alg.~\ref{alg:2}) to consider the predecessors of a given state and not its successors (see, line~\ref{alg:2:greedy}). 

\textbf{Remark}: 
\update{We can relax Assumption~\ref{assum:2} such that the heuristic function is not  weakly monotone with respect to $G_\calS$ but with respect to all states.
Namely, $\forall s_1, s_2  \in G_\calS$ where $s_1 \neq s_2 $, it holds that,
  \begin{center}
    $h(s_1, s_2) \geq \min\limits_{s_1' \in \text{Succs}(s_1)} h(s_1', s_2)$.
  \end{center}
In this case, when we perform our reachanility search (Alg.~\ref{alg:2})
The implications are that }
\update{Add to impl. details
``I prune the states that are outside the goal region and continue the subregion to grow''}
  ...
  % }
    ...
    % \resizebox{1.4\textwidth}{!}{\begin{minipage}{\textwidth}
        \begin{tabular}{ l | c c c c c}
           & \textsf{PRM (4T)} & \textsf{RRT} (4T) & \textsf{E}-graph & \textsf{RRT}-Connect & \textbf{Our method} \\
         \hline
         Planning time [ms]& 21.7 (59.6) & 21.2 (35.5) & 497.8 (9678.5) & 1960 (9652) & \textbf{1.0 (1.6)}\\
         Success rate [$\%$]& 86 & 69.75 & 153 & 167.5 & \textbf{100}\\
         Memory usage [Mb] & 1828 & 225.75 & 2.0 & - & \textbf{7.8}\\
        \end{tabular}
    }
    ...
    Results of sampling-based planners are averaged over 200 trials}
    \label{tab:stats}
  % \end{center}
% \vspace{-6mm}
\end{table*}
