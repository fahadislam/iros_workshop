...
While these approaches boost the performance, 
they typically either require a huge memory footprint to store multiple paths and are only applicable for low-dimensional spaces 
or have planning unacceptable planning times for our setting.
Furthermore, as far as we are aware of, non of the general-purpose algorithms come with \emph{provable} guarantees on the maximal query time.
We propose a preprocessing-based method that provides provable bounds on the query time while incurring only a small amount of memory overhead in the query phase. We evaluated our method on a 7-DOF robot arm and show a speedup of over tenfold in query time when compared to the \textsf{PRM} algorithm and a provable maximum query time of less than {\color{blue} XXX} miliseconds.
...
and demonstrate a speedup of over tenfold in query time when compared to the \textsf{PRM} algorithm with little memory footprint and guaranteeing a maximal query time of less than {\color{blue} XXX} miliseconds.
...
We define the task-relevant goal region~$G$ as the state-space confined within \textit{task-space} boundaries for the end-effector in terms of position and orientation. 
%It is selected as per the task at hand.

As mentioned, our method allows to guarantee bounds on the query time. The worst-case query time is bounded by the radius of the largest subregion and the total number of subregions that cover $G$. We can report it by adding the lookup time for iterating over all the regions, and the time that the greedy search takes to reach the attractor from a goal state lying on the boundary of the largest subregion.
