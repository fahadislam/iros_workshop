...
\begin{enumerate}
  ...
  \begin{center}
  \footnotesize
    $h(s_1, s_2) \geq \min\limits_{\forall s_1' \in Succs(s_1)} h(s_1', s_2) \quad \forall s_1,s_1',s_2 \in G \wedge s_1 \neq s_2$ 
  \end{center}
  Namely, for any distinct start and goal pair ($s_1, s_2$) in $G$, atleast one of $s_1$'s successors (also belonging to $G$) must have a heuristic value less than or equal to its heuristic value. This property should hold assuming that all the edges in $G$ are traversable i.e. they have finite costs.
...
\subsection{Key Idea}
The planner comprises of a preprocessing and a query phase. In the preprocessing phase, $G$ is decomposed into (possibly overlapping) subregions $R_i$ centered around what we call the $attractor$ states $s_{attractor_i}$ and radii $r_i$, where $i$ = 1 to $n$. These regions are constructed in such a way that
\begin{enumerate}
  \item For a goal state $s_{goal} \in R_i$, a greedy search with respect to the heuristic function $h(s, s_{attractor_i})$ will find a path to $s_{attractor_i}$ without getting stuck in local minima.
  \item The union of all the subregions completely cover $G$. In other words, any query state $s_{goal} \in G$ will fall in atleast one of the subregions.
\end{enumerate}
  ...
  \State $s_{nominal}$ $\leftarrow$ GetNominalGoal($G$)
    ...
    \While {$V$ and $I$ are not empty}
        \While {$V$ is not empty}
          \State $s_{attractor_i}$ $\leftarrow$ $V.pop()$ \label{alg:1:pop}
            \If {$s_{attractor_i}$ is not covered}  \label{alg:1:discard}
                \State $\pi_i$ = PlanPath($s_{attractor_i}, s_{start}$)  \label{alg:1:path}
