...
{\color{blue} add related work to real time planning~\cite{KL06,KS09,K90}.}
...
However, storing a path from each $\sStart$ to each state in $G$ is unfeasible.
...
within a known bounded time, we can compute a collision-free path from $\sStart$ to any state in $G$.

\subsection{Key Idea}
Our planner comprises of a preprocessing and a query phase. 
In the preprocessing phase, $G$ is decomposed into a finite  set of (possibly overlapping) subregions $R_i$.
...
\vspace{2mm}        
        ...
        \While {$I$ is not empty} \label{alg:1:iv_loop}
			...
			\If {$s \notin\left( \bigcup_i R_i\right) \cup \left(\bigcup_i\hat{R}_i \right)$ }      \Comment{$s$ is not covered}
