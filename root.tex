...
In manufacturing industry, the robots have to perform highly repetitive manipulation tasks in structured environments. Motion planning in such environments need to be extremely fast to be able to meet high efficiency requirements. Preprocessing-based approaches prove to be very beneficial in these settings---they analyze the state-space offline to generate some auxiliary information which can then be used in the query phase to speedup planning times. 
%
...
\begin{table}[tb]
  \begin{center}
    \begin{tabular}{ l | c c c c}
     ...
     \hline
     Mean Planning time [ms]& - & - & - & 0.48 \\
     Worst-case planning time [ms]& - & - & - & 1.01 \\
     Success rate [$\%$]	& - & - & - & 100 \\
    \end{tabular}
    \caption{Experimental results comparing our method with \textsf{PRM}. The table shows the results for \textsf{PRM} preprocessed with equal, double and quadruple the time that our method takes in precomputation T.}
    \label{tab:stats}
  \end{center}
%  \vspace{-3mm}
\end{table}
...
Preprocessing (Alg.~\ref{alg:1}) took 1128 seconds and returned 1490 subregions. For a fair comparision the \textsf{PRM} was given the same amount of preprocessing time and the paths from all the nodes in $G$ to $s_{\text{start}}$ were precomputed. 
