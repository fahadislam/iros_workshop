...
\hspace*{\algorithmicindent} \textbf{Outputs:} $R_i = (sAttractor, r_i)$, $\pi_i$,
...
\Procedure{PreprocessRegion}{$G, r_{\rm max}$}
  \State $V$ = $\emptyset$   \Comment{list of valid frontier states} \label{alg:1:v}
  \State $I$ = $\emptyset$   \Comment{list of invalid frontier states} \label{alg:1:i}
  \State \sNom $\leftarrow$ GetNominalGoal($G$)
    \State Insert \sNom  in $V$
    \While {$V$ and $I$ are not empty}
        \While {$V$ is not empty}
          \State $\sAttract$ $\leftarrow$ $V.pop()$ \label{alg:1:pop}
            \If {$\sAttract$ is not covered}  \label{alg:1:discard}
                \State $\pi_i$ = PlanPath($\sAttract, \sStart$)  \label{alg:1:path}
                \State $(Open, r)$ $\leftarrow$ ComputeReachability($\sAttract, r_{\rm max}$) \label{alg:1:cr}
